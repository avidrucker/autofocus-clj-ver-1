...
(deftest unit-tests
  (testing "Rendering lists"
    ...
    (is
     (= " - [ ] Wash the dishes"
        ...
        (af/stringify-list [eg/example-item]))
     "Rendering a list with one item works as expected")
    (is
     (= " - [ ] a\n - [ ] b\n - [ ] c"
        (af/stringify-list eg/example-list-all-clean))
     "Rendering a list that has only 
      clean items works as expected")
    (is
     (= "[ ] [ ] [ ]"
        (af/stringify-list-compact eg/example-list-all-clean))
  ...
  (testing "Generating review questions"
    (is (= "Do you want to do 'b' more than 'a'?"
           (af/generate-review-msg eg/example-list-first-marked 1))
        "Generates the correct review message."))

  (testing "Determining if a list is reviewable"
    ;; list of size 0
    (is (= false (af/is-reviewable-list? eg/example-empty-list))
        "Correctly determines list NOT reviewable")
    ;; lists of size 1
    (is (= false (af/is-reviewable-list? eg/example-list-one-item))
        "Correctly determines list NOT reviewable")
    ;; lists of size 2
